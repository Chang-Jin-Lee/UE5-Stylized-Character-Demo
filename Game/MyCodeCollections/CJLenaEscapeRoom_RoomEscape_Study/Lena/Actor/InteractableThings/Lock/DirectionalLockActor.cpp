...
#include "Lena/Actor/InteractableThings/Lock/DirectionalLockActor.h"
...
{
	if(GetNum(InputPassWord) < GetNum(Password)) return;
...
{
	const TObjectPtr<UEnum> EnumPtr = FindObject<UEnum>(ANY_PACKAGE, TEXT("DirectionEnum"), true);
	if (!EnumPtr)
