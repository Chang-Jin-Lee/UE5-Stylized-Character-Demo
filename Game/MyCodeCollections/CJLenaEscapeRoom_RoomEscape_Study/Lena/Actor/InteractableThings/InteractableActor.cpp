...
void AInteractableActor::ZoomInCamera()
{
	if(g_LenaRoom01ManagerIsValid)
	{
		g_LenaRoom01Manager->GetCameraManager()->CreateCameraMoveTargetLocationActor(this);
		g_LenaRoom01Manager->GetCameraManager()->ZoomInCamera();
	}
	IsZoomedIn = true;
}

void AInteractableActor::ZoomOutCamera()
{
	if(g_LenaRoom01ManagerIsValid)
	{
		g_LenaRoom01Manager->GetCameraManager()->ZoomOutCamera();
	}
	IsZoomedIn = false;
}

bool AInteractableActor::IsZoomInEnd()
{
	if(g_LenaRoom01ManagerIsValid)
	{
		return g_LenaRoom01Manager->GetCameraManager()->GetbIsZoomedIn();
	}
	return true;
}